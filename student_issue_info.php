...
<body>
    <div class="request-table">
	    ...
	    if(isset($_SESSION['login_student_username']))
		{
			...
			$q=mysqli_query($db,"SELECT issueid, duedate, student.studentid, books.bookid,books.bookname,books.ISBN,books.bookpic,price,issueinfo.issuedate,issueinfo.returndate,
			issueinfo.approve,fine,authors.authorname,category.categoryname from  `issueinfo` join `books` on issueinfo.bookid=books.bookid join `student`on student.studentid=issueinfo.studentid join authors on authors.authorid=books.authorid join category on category.categoryid=books.categoryid where student.studentid ='$_SESSION[studentid]' and (issueinfo.approve='$var1' or issueinfo.approve='$var' or issueinfo.approve='$var2') ORDER BY `issueinfo`.`returndate` ASC; ");
			...
			else
			{
				...
				$row1=mysqli_query($db,"SELECT  issueid, duedate, sum(fine),student.studentid,fname from issueinfo join student on student.studentid=issueinfo.studentid where student.studentid ='$_SESSION[studentid]' and (issueinfo.approve='$var' OR issueinfo.approve='$var1' OR issueinfo.approve='$var2');");
