...
<body>
    ...
    <div class="request-table">
        <div class="request-container">
            <h2 class="request-title student-info-title">Borrow History</h2>
            <?php
		...
		if(isset($_POST['submit']))
		{
            ...
            SELECT 
                issueid, 
                duedate, 
                IFNULL(CONCAT(student.fname, ' ', student.lname), CONCAT(employee.fname, ' ', employee.lname)) AS user_name,
                IF(student.studentid IS NOT NULL, 'student', 'employee') AS user_type,
                acessionnum, 
                books.bookid, 
                bookname, 
                ISBN, 
                ...
                CASE 
                    WHEN issueinfo.issuedate = '0000-00-00' THEN '2024-01-01' 
                    ELSE issueinfo.issuedate 
                END AS created_at, 
                returndate, 
                approve, 
                fine 
            ...
            WHERE student.fname LIKE '%".$_POST['search']."%' 
               OR student.lname LIKE '%".$_POST['search']."%' 
            AND issueinfo.studentid = '".$_SESSION['studentid']."'
            ORDER BY issueinfo.issuedate ASC
");
		...
		else
		{
			$res = mysqli_query($db, "
                ...
                JOIN authors ON authors.authorid = books.authorid 
                JOIN category ON category.categoryid = books.categoryid 
                WHERE issueinfo.studentid = '".$_SESSION['studentid']."'
                ORDER BY issueinfo.issuedate ASC
            ");
            echo "<table class='rtable'>";
            echo "<tr style='background-color: #1aa7ec;'>";
