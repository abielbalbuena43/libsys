    ...
    <div class="request-table">
        <div class="request-container book-container" id="printTable" style="max-width: 1600px;">
            <div class="student-table-header">
                <h2 class="request-title student-info-title">List of Overdue books</h2>
		...
		if(isset($_POST['submit']))
		{
            ...
            $var3='<p class="expired-pill">UNPAID</p>';
			$q = mysqli_query($db, "
                SELECT issueid, duedate, student.studentid, lrn, acessionnum, fname, studentpic, books.bookid, bookname, ISBN, price, bookpic, authors.authorname, category.categoryname, issueinfo.issuedate, duedate, approve, fine 
                FROM student 
                ...
                WHERE (issueinfo.approve = '$var' OR issueinfo.approve = '$var2') 
                OR (issueinfo.duedate < CURDATE() AND issueinfo.approve = '$var1') 
			...
			else
			{
                ...
                while($row=mysqli_fetch_assoc($q))
                {
                    ...
                    // echo "<td>"; echo $row['approve'] == $var2 ? $row['approve'] : $var3; echo "</td>";
                    ...
                    <?php if($row['lrn'] != ''){ ?>
                    <a href="suspend.php?id=<?php echo $row['studentid'];?>&status=0&page=0"><button style="font-weight:bold;" type="submit" name="submit1" class="delbtn" > Suspend
                        </button>
                    </a>
                    <?php } ?>
			        <?php 
		...
		else
		{
            ...
            while($row=mysqli_fetch_assoc($res))
            {
                ...
                echo "<tr>";
                    ...
                    <a href="edit_issue_book.php?issueid=<?php echo $row['issueid'];?>&ed=<?php echo $row['studentid'];?>&ed1=<?php echo $row['bookid'];?>"><button style="font-weight:bold; width: 100px;" type="submit" name="submit1" class="btn btn-default actionbtn">Edit
			            </button>
                    </a>
                    <?php if($row['lrn'] != ''){ ?>
                    <a href="suspend.php?id=<?php echo $row['studentid'];?>&status=0&page=0"><button style="font-weight:bold;" type="submit" name="submit1" class="delbtn" > Suspend
