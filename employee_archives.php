    ...
    <div class="request-table">
        <div class="request-container">
            <div class="student-table-header">
                ...
                <a href="./employee_info.php"><button class="btn btn-default">Back</button></a>
            </div>
            <?php

		if(isset($_POST['submit']))
		{
			$q=mysqli_query($db,"SELECT * FROM  `employee` where status = 0 AND employee_id ='$_POST[search]';");
			...
			else
			{
                ...
                while($row=mysqli_fetch_assoc($q))
                {
                    ...
                    echo "<td>";?>
                        <a href="archive_employee.php?id=<?php echo $row['employee_id'];?>&status=1"><button style="font-weight:bold;" type="submit" name="submit1" class="btn btn-default" ><i class="fas fa-undo"></i> Restore
                            </button>
                        </a>
                        <a href="delete_employee.php?del=<?php echo $row['employee_id'];?>"><button onclick="del()" style="font-weight:bold;" type="submit" name="submit1" class="delbtn" ><i class="fas fa-trash-alt"></i> Delete
                            </button>
                        </a>
                        <?php
                    echo "</td>"; 
		...
		{
			$res=mysqli_query($db,"SELECT * FROM `employee` WHERE status = 0");
            echo "<table class='rtable'>";
            echo "<tr style='background-color: #1aa7ec;'>";
            //Table header
            echo "<th>"; echo "Students"; echo "</th>";
