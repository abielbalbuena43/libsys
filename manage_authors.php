		...
		if(isset($_POST['submit']))
		{
			...
			else
			{
                ...
                while($row=mysqli_fetch_assoc($q))
                {
                    ...
                    <button onclick="confirmDelete(<?php echo $row['authorid']; ?>)" style="font-weight:bold;" type="submit" name="submit1" class="delbtn" ><i class="fas fa-trash-alt"></i> Delete</button>
			        <?php 
			        echo "</td>";
                    echo "</tr>";
                }
...
function confirmDelete(authorid) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = 'delete_author.php?del=' + authorid;
        }
    })
}
</script>
